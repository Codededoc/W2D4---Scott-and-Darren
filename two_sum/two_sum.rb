...
#
# ### Brute force
# If we weren't worried about time complexity, what would be the most straightforward way
# to solve this problem? To start with, we could check every possible pair.
# If we sum each element with every other, we're sure to either find the pair that sums
...
# Make sure that you don't pair an element with itself.
 # However, you don't need to bother checking for summing the same pair twice; that won't affect your result.
#
# (Note: you can cut the running-time significantly by returning early,
# and by not checking pairs more than once.
...
# Leave that nonsense to the riffraff. Instead, you'll apply a refined and time-honored technique: **sorting**.
#
...
# One way involves using a very cheap algorithm that can only be used on sorted data sets. What are some such algorithms you know?)*
#
# ### Hash Map
...
# and then call over your TA and show them what you've got.
